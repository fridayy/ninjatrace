...
stream_file_close_test() ->
  IoDev = ninjatrace_file:stream_file("/dev/random", read, fun(_X) -> 1+1 end),
  ninjatrace_file:stream_close(IoDev),
  ?assertEqual(erlang:process_info(IoDev), undefined).

stream_file_read_test_() ->
  {setup,
    fun stream_file_read_fixture/0,
    fun stream_file_read_teardown/1,
    fun stream_file_read_exercise/1
  }.

stream_file_read_fixture() ->
...
stream_file_read_teardown({Path, Size}) ->
  file:delete(Path),
  ok.

stream_file_read_exercise({Path, Size}) ->
  Self = self(),
